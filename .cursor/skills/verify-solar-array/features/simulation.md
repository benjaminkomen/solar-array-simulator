...
- `simulation-open` title "Simulation", "Total Output", universal `@expo/ui` season `Picker` (Spring / Summer / Fall / Winter). The selected season is visible; other labels live in the picker menu (not four always-on segments).
...
- Chrome (title / Total Output) must appear even when WebGPU cannot start. A timeout on "Simulation" after tap is a product or navigation issue, not a missing testID. A redbox (`RNWebGPU`) is a regression.

- **Navigate.** `run-flow simulation-nav`: tap "Simulate", wait up to 15s for "Simulation", assert "Total Output", then take `sim-3d-proof` immediately (short settle only) while that chrome is still visible. Do not treat a black canvas as "WebGPU unavailable" when chrome is up.
- **Season.** Open the season picker (`season-picker`) and choose Winter. "Total Output" remains. Time bounds may change with season/location. Maestro may not expand a menu picker — record as `verified-unreachable` unless the change is the picker itself.
...
- **Proof.** `sim-3d-proof` taken while "Simulation" / "Total Output" are still visible — panel/sun or black canvas with chrome. The AVD launcher is not proof. Do not treat a unit test of `getSolarPosition` as this screen.
...
- "Loading 3D scene..." is a Suspense fallback. Waiting only for that string is incomplete — wait for "Total Output".
- Product UI is one `src/app/simulation.tsx`. Controls live in `SimulationControls` (`@expo/ui` `Host` / `Slider` / `Picker`). `SimulationView` stays the WebGPU surface — do not stub it.
- Universal `@expo/ui` `Picker` is menu/wheel only (no segmented appearance). All four season names are no longer simultaneous chrome; `simulation-nav` asserts title + Total Output, not Spring/Summer/Fall/Winter.
...
- "3D view unavailable" is a canvas fallback, not a failed navigation. Maestro should still see "Simulation" / "Total Output".
