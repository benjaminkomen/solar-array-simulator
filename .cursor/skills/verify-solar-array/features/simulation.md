...
- `react-native-wgpu@0.4.x` throws `Property 'RNWebGPU' doesn't exist` if imported before native `install()`. 0.4.x `install()` also fails on Expo 56 bridgeless (`RCTCxxBridge` / `getCatalystInstance()`). This app uses `react-native-webgpu@0.10` plus a SimulationView probe. After the native upgrade, rebuild `development-simulator` (iOS) / `development` (Android).
