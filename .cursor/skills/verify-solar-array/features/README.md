...
Surface: iOS Simulator and Android emulator development builds. Maestro YAML in `.maestro/` branches by platform (`launch-fresh.ios.yaml` / `launch-fresh.android.yaml`, plus shared tap helpers). Remaining platform-stub routes (`src/app/*.tsx` without `.ios` / `.android`, except Production and compass-help) show "not yet implemented" — do not treat those as the product. `src/app/production.tsx` is the universal Production route. `src/app/compass-help.tsx` is the universal help body (chrome stays in `_layout`).
