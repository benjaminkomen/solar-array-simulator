...
- Android Finish matches Analyze Skip: `Stack.Toolbar.View hidden={!shouldShowWizardFinish}` + RN `Pressable` wrapping the `Finish` label (`toolbarFinishHit`). Link/Rotate/Delete stay mounted with `hidden={!selectedId}` so Add cannot shift Finish's toolbar index. The Pressable `onPress` is a stable callback that reads `pressWizardFinish` refs — do **not** `if (visible)` in the handler (Compose can keep the empty-canvas closure, so the first Finish tap is a no-op). Do not `disabled={!visible}`. Do **not** use Add's overlay-on-text for Finish.
- Finish `onPress` resets the root stack to Production (`dispatchWizardFinish`). Do **not** `router.push` / `navigation.navigate` / same-tap retry — after one tap Custom can stay focused with Production underneath (wizard step 3 + FINISH, no Total Array Output). Persist `wizardCompleted` in `useProductionMonitor`.
