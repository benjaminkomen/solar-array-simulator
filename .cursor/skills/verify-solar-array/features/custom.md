...
- Android Finish matches Upload/Analyze Skip: `Stack.Toolbar.View` + RN `Pressable` wrapping the `Finish` label (`toolbarFinishHit`, 48×48). The Toolbar.View stays mounted for the whole Custom visit; `visible={shouldShowWizardFinish}` only reveals the text/a11y. Do **not** use Add's overlay-on-text (`AndroidToolbarHitOverlay`) for Finish — Maestro `tapOn: "Finish"` prefers the uppercase Text, and a newly mounted Host after Add is a dead first tap (Mac #71 @ `94497c6`: first Finish left Custom; a later tap reached Production).
- Finish `onPress` only `expoRouter.push('/production')` (imperative `router` from `expo-router`, not the `useRouter()` instance), deferred with `setTimeout(0)`. Do **not** write `wizardCompleted` in that handler. A sync store notify or a push during the Android Host press re-renders Custom/Welcome and Expo `routingQueue` drops the first action (`ref.current` null). Persist the flag in `useProductionMonitor` via `persistWizardCompletedOnProduction`.
