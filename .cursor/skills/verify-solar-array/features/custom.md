...
- Android Finish is a real RN `Pressable` (`AndroidWizardFinishButton`) **outside** `Stack.Toolbar`, above the 64dp Compose Host (`androidWizardFinishBottom`) and **left of** the zoom pill (`androidWizardFinishRight`). SHA1-identical Layout+FINISH after tap means `onPress` did not run — if it had, Custom would render only `<Redirect>` and unmount Layout. The leftover-toolbar hypothesis is closed; the zoom column (RNGH Pressable + elevation) was eating the hit. Exactly one `accessibilityLabel="Finish"`. Do **not** mount `WizardFinishButton` on Android. iOS Finish stays `Stack.Toolbar.Button`. Do **not** add a second Maestro Finish tap. Do **not** stack `navigate` / `reset` / retry.
