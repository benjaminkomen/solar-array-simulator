...
- iOS header-right must show **three** items: Toggle compass, link + Badge, Snap to origin. Expo Router's iOS `processHeaderItemsForPlatform` keeps only direct `Stack.Toolbar.Button` / Menu / Spacer / View children (`isChildOfType`). Wrapping compass/snap in `ToolbarIconButton` drops them and leaves only the Badge link. Android wrappers stay — header-right is a Compose Row. Bottom toolbar wrappers are fine on iOS (React children, not header items).
- `wizard-happy-path` and `full-app-tour` assert Toggle compass + Snap to origin on the empty canvas and again after Add (Badge case). iOS-only: "Unlinked panels" after Add. Snap tap must leave `canvas-container` visible (recenter is Skia — not Maestro-visible).
