...
- Android Finish is a real RN `Pressable` (`AndroidWizardFinishButton`) **outside** `Stack.Toolbar`, sitting above the 64dp Compose `RouterToolbarHost` (`androidWizardFinishBottom`). Do **not** mount `WizardFinishButton` as a toolbar child on Android — a `null` slot can leave a leftover FINISH a11y node that Maestro hits instead of the Pressable (#80). Exactly one `accessibilityLabel="Finish"`. iOS Finish stays `Stack.Toolbar.Button`. Do **not** use Add's overlay-on-text for Finish. Do **not** add a second Maestro Finish tap.
