...
- Android Finish position is locked (above Host, left of zoom). Do **not** move it again. On press the **same** Pressable flips its visible text and a11y label from Finish → `Tapped` (`androidWizardFinishPressProofLabel`) before Redirect. Next drive: Layout + `TAPPED` means onPress ran and Redirect failed; Layout + `FINISH` (SHA1-identical) means `tapOn: Finish` is not this Pressable. Production / Total Array Output means both ran.
- Android a11y nodes matching Finish / FINISH before press (`listAndroidFinishA11yNodes`): **exactly one** — `AndroidWizardFinishButton` `accessibilityLabel="Finish"`. Inner `Text` is `accessible={false}` / `importantForAccessibility="no"`. iOS `WizardFinishButton` children `Finish` is **not** mounted on Android (`Platform.OS !== "android"`). Do **not** add a second `accessibilityLabel="Finish"`. Official happy path stays one `tapOn: "Finish"`. Do **not** stack `navigate` / `reset` / retry.
