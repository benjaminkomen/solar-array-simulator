...
- Android Finish is a real RN `Pressable` (`AndroidWizardFinishButton`) **outside** `Stack.Toolbar`. Compose Host / Toolbar.View ate every in-toolbar Finish we tried (dead node, stale `visible`, `hidden` slot, `navigate`/`reset`). iOS Finish stays `Stack.Toolbar.Button`. Do **not** use Add's overlay-on-text for Finish.
