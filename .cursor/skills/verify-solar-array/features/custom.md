...
- Android Finish position is locked. Do **not** move it again. Visual is Skia (`AndroidWizardFinishGlyph.android.tsx`), not RN `Text`. `94bd4e5` uiautomator had **2** Finish nodes: Pressable `android-wizard-finish` and a FINISH TextView. Maestro `tapOn: Finish` hit the text. `accessible={false}` is not enough. Official happy path stays one `tapOn: "Finish"` and must land on the Pressable (`content-desc=Finish` / id `android-wizard-finish`). On press the same control flips to `Tapped` then Redirect. Layout + `TAPPED` = press ran, Redirect failed. Layout + `FINISH` = tap still missed the Pressable. Production = both ran. Do not change `wizard-happy-path` to the testID unless the product label is gone.
