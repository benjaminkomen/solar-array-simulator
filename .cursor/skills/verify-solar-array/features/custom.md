...
- Android Finish matches Upload/Analyze Skip: `Stack.Toolbar.View` + RN `Pressable` wrapping the `Finish` label (`toolbarFinishHit`, 48×48). The Toolbar.View stays mounted for the whole Custom visit; `visible={shouldShowWizardFinish}` only reveals the text/a11y. The Pressable stays enabled (do not `disabled={!visible}` — Android can swallow the first press after enable). Do **not** use Add's overlay-on-text for Finish.
- Finish `onPress` dispatches `navigation.navigate('production')` on the focused stack (`dispatchWizardFinish`). Do **not** `router.push` / `setTimeout(0)` — Expo `routingQueue` + `useImperativeApiEmitter` can drop the first Android action after Add (`ref.current` null or a same-snapshot store miss). Mac #71 @ `64fe44a`: Pressable + Finish a11y were live; first tap left Custom; second tap reached Production. Android retries once on the next frame if Custom is still focused (`retryWizardFinishIfNeeded`). Persist `wizardCompleted` in `useProductionMonitor`.
