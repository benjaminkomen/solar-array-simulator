...
- **Model list + Skip.** `run-flow analyze-skip` after a gallery pick: wait for "Select AI Model", assert "Claude Sonnet 4.6 (Default)", "Claude Opus 4.6", "Amazon Nova Pro", tap "Skip", wait for `id: canvas-container`.
...
- Shared `analyze.tsx` is a stub. iOS copy is "Select AI Model"; Android is "SELECT AI MODEL". `analyze-skip.yaml` uses the iOS string.
