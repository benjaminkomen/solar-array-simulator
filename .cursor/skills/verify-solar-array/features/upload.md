...
Wizard step 2 (`/upload?wizard=true`): take a photo, pick from the gallery, Skip, or Continue without photo. A successful pick navigates to Analyze; Skip goes to Custom.

One route: `src/app/upload.tsx` (#59). Do not paper this as still-split. No Host / Reanimated `entering` on first paint.
...
- **Gallery → Analyze (iOS).** `run-flow analyze-skip`: tap `id: choose-gallery-button`, wait for system "Photos", tap `point: 17%,25%` (first grid tile), wait for **Select AI Model**.
- **Continue without photo (Android).** `run-flow analyze-skip` taps `id: analyze-empty-state-button`. Do not open the system picker on an empty AVD. Assert **No photo selected** on Analyze.
...
- Wizard-only **Continue without photo** (`id: analyze-empty-state-button`) opens Analyze empty-state. `analyze-skip` iOS still uses Photos (`Photos` + `17%,25%`). Android must tap that fixture.
