...
- **Add inverter.** Tap toolbar control with accessibilityLabel "Add inverter". iOS is `Stack.Toolbar.Button` (SF `plus` may also appear as `add`). Android is `Stack.Toolbar.View` + RN `Pressable` (`AndroidToolbarIconButton`) — do not tap a Compose Icon. Sheet title is "New Micro-inverter".
...
- Android Add inverter is `AndroidToolbarIconButton` (`Stack.Toolbar.View` + RN `Pressable`, `accessibilityLabel="Add inverter"`). iOS stays `Stack.Toolbar.Button` with SF `plus`. Do not put `accessibilityLabel` on a Compose `Icon`.
