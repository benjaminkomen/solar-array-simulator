...
- `analyze-skip` iOS still uses Photos chrome (`Photos` + `17%,25%`). Android must **not** open the system picker on an empty AVD — tap `id: analyze-empty-state-button` ("Continue without photo") and prove **Select AI Model** + **No photo selected**.
