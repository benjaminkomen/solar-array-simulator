...
- **Android** (`launch-fresh.android.yaml`): Maestro `openLink` to `solararraysimulator://expo-development-client/?url=http%3A%2F%2F10.0.2.2%3A8081` (`10.0.2.2` = AVD host loopback — `127.0.0.1` inside the emulator is the guest). Optional Continue/Reload, wait for Welcome. Override with `-e METRO_URL=…` (physical device or `adb reverse tcp:8081 tcp:8081`).
...
- iOS `launch-fresh`: deep-link `127.0.0.1:8081`, tap **Open** (not Cancel), wait for **Continue**. Android: `openLink` to `10.0.2.2:8081` (`disableOnboarding=1`). Optional Reload dismiss still uses `50%,15%`.
