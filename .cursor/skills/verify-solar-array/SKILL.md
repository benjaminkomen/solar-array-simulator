...
- Skia/WebGPU pixels are not queryable. Prove Custom via `canvas-container` + toolbar (`Finish` after add). Prove Simulation via chrome (`Simulation`, `Total Output`) and `sim-3d-proof` taken while that chrome is still in the foreground. A black canvas with chrome up is not "WebGPU unavailable"; the AVD launcher is not Simulation proof.
