...
| Custom canvas / toolbar / compass | `wizard-happy-path` | Shared `CustomChrome` (#62). Android Add works. iOS header-right is three direct `Stack.Toolbar.Button`s (compass, link+`Badge`, snap) — wrappers are dropped by the iOS header-item filter. Android header-right is Pressable + RN count badge. Skia is not Maestro-visible — prove via toolbar side effects. Wizard Finish stays hidden until the first panel exists. |
